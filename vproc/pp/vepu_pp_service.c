...
enum PP_RET pp_service_init(void *ctx, int type)
{
    ...
    if (!p->chnl) {
        ret = VEPU_PP_NOK;
        pr_err("pp_service_init mpp_chnl_open type %d fail\n", type);
    } else {
        if (p->chnl->mpp && p->chnl->mpp->dev)
            p->dev = p->chnl->mpp->dev;
    }
    return ret;
}
...
struct device *pp_service_get_dev(void *ctx)
{
    struct vepu_pp_dev_srv *p = (struct vepu_pp_dev_srv *) ctx;
...
const struct pp_srv_api_t pp_srv_api = {
    .name               = "vepu_pp",
    .ctx_size           = sizeof(struct vepu_pp_dev_srv),
    .init               = pp_service_init,
    .deinit             = pp_service_deinit,
    .reg_wr             = pp_service_reg_wr,
    .reg_rd             = pp_service_reg_rd,
    .cmd_send           = pp_service_cmd_send,
    .cmd_poll           = pp_service_cmd_poll,
    .get_address        = pp_service_iova_address,
    .release_address    = pp_service_iova_release,
    .get_dev            = pp_service_get_dev,
};
