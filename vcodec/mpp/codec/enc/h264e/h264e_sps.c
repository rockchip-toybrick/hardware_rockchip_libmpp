...
MPP_RET h264e_sps_to_packet(H264eSps *sps, KmppPacket packet, RK_S32 *len)
{
	KmppShmPtr pos;
	KmppShmPtr data;
	RK_S32 size;
	...
	RK_S32 sps_size = 0;
	static const RK_U32 sub_width[4]  = { 1, 2, 2, 1};
	static const RK_U32 sub_height[4] = { 1, 2, 1, 1};
	...
	if (sps->cropping) {
		...
		mpp_writer_put_ue(bit, sps->crop.left / sub_width[sps->chroma_format_idc]);
		/* frame_crop_right_offset */
		mpp_writer_put_ue(bit, sps->crop.right / sub_width[sps->chroma_format_idc]);
		/* frame_crop_top_offset */
		mpp_writer_put_ue(bit, sps->crop.top / sub_height[sps->chroma_format_idc]);
		/* frame_crop_bottom_offset */
		mpp_writer_put_ue(bit, sps->crop.bottom / sub_height[sps->chroma_format_idc]);
	}
	...
	kmpp_packet_set_length(packet, length + sps_size);
