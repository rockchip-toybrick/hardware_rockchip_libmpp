...
//   mpp_env_get_u32("h265e_debug", &h265e_debug, 0);
	...
	h265->trans_cfg.diff_cu_qp_delta_depth = 1;
...
static MPP_RET h265e_proc_h265_cfg(MppEncH265Cfg * dst, MppEncH265Cfg * src)
{
	...
	if (change & MPP_ENC_H265_CFG_TRANS_CHANGE) {
		...
		if (src->trans_cfg.diff_cu_qp_delta_depth > 2 || src->trans_cfg.diff_cu_qp_delta_depth < 0) {
			...
			src->trans_cfg.diff_cu_qp_delta_depth = 1;
		}
...
static void h265e_proc_show(void *seq_file, void *ctx, RK_S32 chl_id)
{
	...
	seq_puts(seq,
		 "--------Syntax INFO2------------------------------------------------------------------------------\n");
	seq_printf(seq, "%7s|%10s|%8s|%8s|%10s|%10s|%15s|%12s|%8s\n",
		   "ID", "DblkEn",
		   "Tc", "Beta",
		   "Saoluma", "Saochroma",
		   "IntraSmoothing", "ScalingMode", "TmvpEn");
	seq_printf(seq, "%7d|%10s|%8d|%8d|%10d|%10d|%15d|%12d|%8d\n",
