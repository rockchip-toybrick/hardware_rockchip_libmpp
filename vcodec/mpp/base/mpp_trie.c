...
static MPP_RET mpp_trie_check(MppTrie trie, const char *log)
{
    ...
    for (i = 0; i < p->info_used; i++) {
        ...
        mpp_err_f("trie check on %s found mismatch info %s %d - %d\n",
...
MPP_RET mpp_trie_last_info(MppTrie trie)
{
    ...
    if (!buf) {
        mpp_err_f("failed to alloc trie buffer size %d\n", len);
        return MPP_NOK;
    }
...
MPP_RET mpp_trie_import(MppTrie trie, void *root)
{
    ...
    for (i = 0; i < p->info_used; i++) {
        ...
        mpp_err_f("trie check on import found mismatch info %s [%d:%p] - [%d:%p]\n",
...
void mpp_trie_dump(MppTrie trie, const char *func)
{
    ...
    mpp_dbg(0xff, "%s dumping trie %p\n", func, trie);
    mpp_dbg(0xff, "name %s size %d node %d info %d\n",
    ...
    for (i = 0; i < p->node_used; i++) {
        ...
        if (node->id >= 0) {
            ...
            if (node->id < (RK_S32)(p->node_used * sizeof(MppTrieNode)))
                mpp_dbg(0xff, "node %d key %x info %d - %s\n", node->idx, node->key, node->id,
            ...
            mpp_dbg(0xff, "node %d key %x\n", node->idx, node->key);

        if (node->tag_len)
            mpp_dbg(0xff, "    prev %d count %d tag %d - %llx\n", node->prev, node->next_cnt, node->tag_len, node->tag_val);
        else
            mpp_dbg(0xff, "    prev %d count %d\n", node->prev, node->next_cnt);

        for (j = 0; j < MPP_TRIE_KEY_MAX; j++) {
            if (node->next[j] > 0) {
                mpp_dbg(0xff, "    next %d:%d -> %d\n", valid_count, j, node->next[j]);
                valid_count++;
            }
        }
    ...
    mpp_dbg(0xff, "node | next |  tag | used %d\n", p->node_used);

    for (i = 0; i < 17; i++) {
        if (next_cnt[i] || tag_len[i])
            mpp_dbg(0xff, "%2d   | %4d | %4d |\n", i, next_cnt[i], tag_len[i]);
    }

    mpp_dbg(0xff, "%s dumping %d node end\n", func, p->node_used);
}