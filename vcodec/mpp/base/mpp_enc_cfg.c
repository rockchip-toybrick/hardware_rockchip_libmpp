...
#define ENTRY_TABLE(ENTRY)                                                                                                                                          \
    ...
    ENTRY(rc,   fps_in_denorm,  S32,            MPP_ENC_RC_CFG_CHANGE_FPS_IN,           rc, fps_in_denorm)                                                  \
    ...
    ENTRY(rc,   fps_out_denorm, S32,            MPP_ENC_RC_CFG_CHANGE_FPS_OUT,          rc, fps_out_denorm)                                                 \
    ...
    ENTRY(rc,   fm_lvl_qp_min_i,    S32,        MPP_ENC_RC_CFG_CHANGE_FM_LV_QP,         rc, fm_lvl_qp_min_i)                                                \
    ENTRY(rc,   fm_lvl_qp_min_p,    S32,        MPP_ENC_RC_CFG_CHANGE_FM_LV_QP,         rc, fm_lvl_qp_min_p)                                                \
    ENTRY(rc,   fm_lvl_qp_max_i,    S32,        MPP_ENC_RC_CFG_CHANGE_FM_LV_QP,         rc, fm_lvl_qp_max_i)                                                \
    ENTRY(rc,   fm_lvl_qp_max_p,    S32,        MPP_ENC_RC_CFG_CHANGE_FM_LV_QP,         rc, fm_lvl_qp_max_p)                                                \
    ENTRY(rc,   mt_st_swth_frm_qp,  S32,        MPP_ENC_RC_CFG_CHANGE_FM_LV_QP,         rc, mt_st_swth_frm_qp)                                              \
    /* prep config */                                                                                                                                               \
    ...
    ENTRY(prep, rotation,       S32,            MPP_ENC_PREP_CFG_CHANGE_ROTATION,       prep, rotation)                                                     \
    ENTRY(prep, mirroring,      S32,            MPP_ENC_PREP_CFG_CHANGE_MIRRORING,      prep, mirroring)                                                    \
    /* codec coding config */                                                                                                                                       \
    ...
    ENTRY(h264, cabac_en,       S32,            MPP_ENC_H264_CFG_CHANGE_ENTROPY,        codec.h264, entropy_coding_mode)                                    \
    ENTRY(h264, cabac_idc,      S32,            MPP_ENC_H264_CFG_CHANGE_ENTROPY,        codec.h264, cabac_init_idc)                                         \
    ENTRY(h264, trans8x8,       S32,            MPP_ENC_H264_CFG_CHANGE_TRANS_8x8,      codec.h264, transform8x8_mode)                                      \
    ...
    ENTRY(h265, scaling_list,   U32,            MPP_ENC_H265_CFG_TRANS_CHANGE,                  codec.h265, trans_cfg.scaling_list_mode)                    \
    ...
    ENTRY(tune, lambda_idx,     S32,            MPP_ENC_TUNE_CFG_CHANGE_LAMBDA_IDX,     tune, lambda_idx)                                                   \
    ENTRY(tune, lambda_i_idx,   S32,            MPP_ENC_TUNE_CFG_CHANGE_LAMBDA_I_IDX,   tune, lambda_i_idx)                                                 \
    ENTRY(tune, atf_str,        S32,            MPP_ENC_TUNE_CFG_CHANGE_ATF_STR,        tune, atf_str)

#endif

static MppCfgInfoHead mHead;
static MppTrie mTrie;

static MppTrieInfo *get_info(const char *name)
{
    return mpp_trie_get_info(mTrie, name);
}

static MppTrieInfo *get_info_first(void)
{
    if (NULL == mTrie)
        return NULL;
...
static MppTrieInfo *get_info_next(MppTrieInfo *node)
{
    if (NULL == mTrie)
        return NULL;
...
static RK_S32 get_node_count(void)
{
    return mHead.node_count;
}
static RK_S32 get_info_count(void)
{
    return mHead.info_count;
}
static RK_S32 get_info_size(void)
{
    return mHead.info_size;
}
// static RK_S32 get_cfg_size() { return mCfgSize; };
...
{
    MppTrieInfo *root = get_info_first();
    ...
    mpp_log("total cfg count %d with %d node size %d\n",
            get_info_count(), get_node_count(), get_info_size());
}
