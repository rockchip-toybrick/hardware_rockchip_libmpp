...
static MPP_RET hal_jpege_vpu720_init(void *hal, MppEncHalCfg *cfg)
{
    ...
    ret = mpp_buffer_get(NULL, &ctx->qtbl_buffer, JPEGE_VPU720_QTABLE_SIZE * sizeof(RK_U16));
...
static MPP_RET jpege_vpu720_setup_format(void *hal, HalEncTask *task)
{
    ...
    ctx->fmt_cfg.dst_range = ctx->fmt_cfg.src_range;
    hal_jpege_leave();
    return MPP_OK;
}
...
MPP_RET hal_jpege_vpu720_gen_regs(void *hal, HalEncTask *task)
{
    ...
    reg_base->reg011_wdg_jpeg = 0xffffffff;//syntax->mcu_cnt * 1000;
    ...
    reg_base->reg032_sw_src_fmt.u_force_value = 0;//ctx->fmt_cfg.fix_chroma_u;
    reg_base->reg032_sw_src_fmt.v_force_value = 0;//ctx->fmt_cfg.fix_chroma_v;
    ...
    iova = mpp_buffer_get_iova(task->input, ctx->dev);
    reg_base->reg022_adr_src0 = iova;
    reg_base->reg023_adr_src1 = iova + ctx->fmt_cfg.u_offset;
    reg_base->reg024_adr_src2 = iova + ctx->fmt_cfg.v_offset;

    iova = mpp_buffer_get_iova(task->output->buf, ctx->dev);
    ...
    mpp_buffer_flush_for_device(ctx->qtbl_buffer);
