...
static MPP_RET vepu510_h264e_use_pass1_patch(HalVepu510RegSet *regs, HalH264eVepu510Ctx *ctx)
{
	MppEncPrepCfg *prep = &ctx->cfg->prep;
	H264eVepu510Frame *reg_frm = &regs->reg_frm;
	RK_U32 iova = mpp_buffer_get_iova(ctx->buf_pass1, ctx->dev);
	RK_S32 y_stride;
	RK_S32 c_stride;
	...
	reg_frm->common.adr_src0   = iova;
	reg_frm->common.adr_src1   = iova + 2 * y_stride;
	reg_frm->common.adr_src2   = iova + 2 * y_stride;
				...
				H264ePps *pps, H264eSlice *slice)
{
	...
	reg_frm->synt_sli1.idr_pid        = (slice->slice_type == H264_I_SLICE) ? slice->idr_pic_id :
					    (RK_U32)(-1);
				   ...
				   H264ePps *pps, H264eSlice *slice)
{
	...
	reg_frm->rdo_cfg.scl_lst_sel    = pps->scaling_list_mode;
	reg_frm->rdo_cfg.atf_e          = ctx->cfg->tune.atf_str > 0;
	reg_frm->rdo_cfg.atr_e          = ctx->cfg->tune.atf_str > 0;
...
static void setup_vepu510_rc_base(HalVepu510RegSet *regs, HalH264eVepu510Ctx *ctx,
				  EncRcTask *rc_task)
{
				 ...
				 HalEncTask *task)
{
	...
	reg_frm->common.adr_bsbs   = fd_out + ((buf_out->start_offset + off_out) % size_out);
	reg_frm->common.bsbr_addr  = fd_out + buf_out->r_pos;
...
static void setup_vepu510_recn_refr(HalH264eVepu510Ctx *ctx, HalVepu510RegSet *regs)
{
	...
	if (curr && curr->cnt) {
		...
		RK_S32 fd = mpp_buffer_get_iova(buf_pixel, ctx->dev);

		mpp_assert(buf_pixel);
		mpp_assert(buf_thumb);

		reg_frm->common.rfpw_h_addr = fd;
		reg_frm->common.rfpw_b_addr = fd + fbc_hdr_size;
		reg_frm->common.dspw_addr = mpp_buffer_get_iova(buf_thumb, ctx->dev);
		reg_frm->common.adr_smear_wr = mpp_buffer_get_iova(buf_smear, ctx->dev);
	}
	...
	if (refr && refr->cnt) {
		...
		reg_frm->common.rfpr_b_addr = fd + fbc_hdr_size;
		reg_frm->common.dspr_addr = mpp_buffer_get_iova(buf_thumb, ctx->dev);
		reg_frm->common.adr_smear_rd = mpp_buffer_get_iova(buf_smear, ctx->dev);
	}
...
static void setup_vepu510_ext_line_buf(HalVepu510RegSet *regs, HalH264eVepu510Ctx *ctx)
{
	...
	fd = mpp_buffer_get_iova(ctx->ext_line_buf, ctx->dev);
	offset = ctx->ext_line_buf_size;

	reg_frm->common.ebuft_addr = fd + offset;
...
static MPP_RET hal_h264e_vepu510_gen_regs(void *hal, HalEncTask *task)
{
	...
	setup_vepu510_io_buf(regs, ctx, task);
