...
static void vepu510_h265_rdo_cfg(H265eV510HalContext *ctx, H265eVepu510Sqi *reg, MppEncSceneMode sm)
{
	...
	if (ctx->frame_type == INTRA_FRAME)
		vepu510_h265_set_atr_regs(reg, sm, 0/* ctx->cfg->tune.atr_str_i */);
	else
		vepu510_h265_set_atr_regs(reg, sm, 0/* ctx->cfg->tune.atr_str_p */);

	if (ctx->frame_type == INTRA_FRAME)
		vepu510_h265_set_anti_blur_regs(reg, 0/* ctx->cfg->tune.sao_str_i */);
	else
		vepu510_h265_set_anti_blur_regs(reg, 0/* ctx->cfg->tune.sao_str_p */);
}
...
static void vepu510_h265_smear_cfg(H265eVepu510Sqi *reg, H265eV510HalContext *ctx)
{
	...
	RK_S32 deblur_en  = 0;//ctx->cfg->tune.deblur_en;
	RK_S32 deblur_str = 0;//ctx->cfg->tune.deblur_str;
	RK_S16 flag_cover = 0;
	RK_S16 flag_bndry = 0;
	...
	reg->subj_opt_inrar_coef.common_rdo_cu_intra_r_coef_dep0    = smear_common_intra_r_dep0[deblur_str]
								      + smear_flag_cover_intra_wgt0[flag_bndry];
	reg->subj_opt_inrar_coef.common_rdo_cu_intra_r_coef_dep1    = smear_common_intra_r_dep1[deblur_str]
								      + smear_flag_cover_intra_wgt1[flag_bndry];

	/* anti smear */
	reg->smear_opt_cfg0.anti_smear_en               = 1;
	if (deblur_en == 0)
		reg->smear_opt_cfg0.anti_smear_en           = 0;
	reg->smear_opt_cfg0.smear_strength              = smear_strength[deblur_str] +
							  smear_flag_bndry_wgt[deblur_en];
	...
	if (((frame_num) % frame_keyint == 0) || reg->smear_opt_cfg0.frm_static == 0
	    || (frame_num) % frame_keyint == 1) {
	...
	if (((frame_num) % frame_keyint == 0) || reg->smear_opt_cfg0.frm_static == 0
	    || (frame_num) % frame_keyint == frame_keyint - 1) {
	...
	reg->smear_madp_cov_thd.smear_qp_strength       = smear_qp_strength[deblur_str] +
							  smear_flag_cover_wgt[flag_cover];
	reg->smear_madp_cov_thd.smear_thre_qp           = 30;

	reg->subj_opt_dqp1.bndry_rdo_cu_intra_r_coef_dep0   = smear_bndry_intra_r_dep0[deblur_str] +
							      smear_flag_bndry_intra_wgt0[flag_bndry];
	reg->subj_opt_dqp1.bndry_rdo_cu_intra_r_coef_dep1   = smear_bndry_intra_r_dep1[deblur_str] +
							      smear_flag_bndry_intra_wgt1[flag_bndry];
}
...
static void vepu510_h265_global_cfg_set(H265eV510HalContext *ctx, H265eV510RegSet *regs)
{
	...
	RK_S32 atf_str = 0;//ctx->cfg->tune.anti_flicker_str;
	RK_S32 lambda_idx = 0;

	vepu510_h265_rdo_cfg(ctx, reg_sqi, sm);
	vepu510_h265_atf_cfg(reg_sqi, atf_str);
	vepu510_h265_smear_cfg(reg_sqi, ctx);
	memcpy(&reg_param->pprd_lamb_satd_0_51[0], lamd_satd_qp_510, sizeof(lamd_satd_qp_510));
...
static MPP_RET vepu510_h265_set_patch_info(H265eSyntax_new *syn, VepuFmt input_fmt,
					   H265eV510HalContext *ctx, HalEncTask *task)
{
	...
	// }
	regs->reg_frm.common.adr_src1 += u_offset;
	regs->reg_frm.common.adr_src2 += v_offset;
...
static MPP_RET vepu510_h265_set_rc_regs(H265eV510HalContext *ctx, H265eV510RegSet *regs,
					HalEncTask *task)
{
...
static MPP_RET vepu510_h265_set_pp_regs(H265eV510RegSet *regs, VepuFmtCfg *fmt,
					MppEncPrepCfg *prep_cfg)
{
	...
	reg_frm->common.src_fmt.out_fmt = ((prep_cfg->format & MPP_FRAME_FMT_MASK) == MPP_FMT_YUV400) ? 0 :
					  1;
	...
	stridec = (reg_frm->common.src_fmt.src_cfmt == VEPU541_FMT_YUV420SP ||
		   ...
		   reg_frm->common.src_fmt.src_cfmt == VEPU540C_FMT_YUV444P) ?
		  stridey : stridey / 2;

	if (reg_frm->common.src_fmt.src_cfmt == VEPU540C_FMT_YUV444SP)
		stridec = stridey * 2;

	if (reg_frm->common.src_fmt.src_cfmt < VEPU541_FMT_ARGB1555) {
...
static void vepu510_h265_set_me_regs(H265eV510HalContext *ctx, H265eSyntax_new *syn,
				     H265eVepu510Frame *regs)
{
...
void vepu510_h265_set_hw_address(H265eV510HalContext *ctx, H265eVepu510Frame *regs,
				 HalEncTask *task)
{
	...
	regs->common.adr_src0  = mpp_buffer_get_iova(enc_task->input, ctx->dev);
	...
	if (!syn->sp.non_reference_flag) {
		regs->common.rfpw_h_addr  = mpp_buffer_get_iova(recon_buf->buf[0], ctx->dev);
		regs->common.rfpw_b_addr  = regs->common.rfpw_h_addr + ctx->fbc_header_len;
	}
	regs->common.rfpr_h_addr = mpp_buffer_get_iova(ref_buf->buf[0], ctx->dev);
	regs->common.rfpr_b_addr = regs->common.rfpr_h_addr + ctx->fbc_header_len;
	regs->common.colmvw_addr = mpp_buffer_get_iova(recon_buf->buf[2], ctx->dev);
	regs->common.colmvr_addr = mpp_buffer_get_iova(ref_buf->buf[2], ctx->dev);
	regs->common.dspw_addr = mpp_buffer_get_iova(recon_buf->buf[1], ctx->dev);
	regs->common.dspr_addr = mpp_buffer_get_iova(ref_buf->buf[1], ctx->dev);

	if (md_info_buf) {
		regs->common.enc_pic.mei_stor = 1;
		regs->common.meiw_addr = mpp_buffer_get_iova(md_info_buf, ctx->dev);
	} else {
...

	{
		...
		regs->common.adr_bsbs  = iova + ((enc_task->output->start_offset + len) % size);
		if (len && enc_task->output->buf) {
			enc_task->output->use_len = len;
			mpp_ring_buf_flush(enc_task->output, 0);
		}
	}
	...
	regs->common.adr_smear_wr = mpp_buffer_get_iova(recon_buf->buf[3], ctx->dev);
}
					      ...
					      RK_S32 tiles_enabled_flag)
{
	...
	reg_frm->common.rfpw_h_addr = mpp_buffer_get_iova(ctx->buf_pass1, ctx->dev);
...
static MPP_RET vepu510_h265e_use_pass1_patch(H265eV510RegSet *regs, H265eV510HalContext *ctx)
{
	...
	reg_frm->common.adr_src0 = mpp_buffer_get_iova(ctx->buf_pass1, ctx->dev);
	reg_frm->common.adr_src1 = reg_frm->common.adr_src0 + 2 * hor_stride;
...
static void setup_vepu510_ext_line_buf(H265eV510HalContext *ctx, H265eV510RegSet *regs)
{
	...
	if (ctx->ext_line_buf) {
		RK_U32 iova = mpp_buffer_get_iova(ctx->ext_line_buf, ctx->dev);

		reg_frm->common.ebufb_addr = iova;
		reg_frm->common.ebuft_addr = iova + ctx->ext_line_buf_size;
	} else {
...
MPP_RET hal_h265e_v510_gen_regs(void *hal, HalEncTask *task)
{
	...
	reg_frm->common.enc_pic.cur_frm_ref         =
	...
	reg_frm->rdo_cfg.scl_lst_sel    = syn->pp.scaling_list_mode;
	{
		RK_U32 i_nal_type = 0;
	...
	ret = vepu510_h265_set_patch_info(syn, (VepuFmt)fmt->format, ctx, enc_task);
	if (ret)
		return ret;
...
static MPP_RET vepu510_h265_set_feedback(H265eV510HalContext *ctx, HalEncTask *enc_task)
{
	...
	Vepu510H265eFrmCfg *frm = ctx->frms[0/* enc_task->flags.reg_idx */];
