...
int mpp_enc_cfg_setup(KmppChanId id)
{
    RK_U32 width = 1280;
    RK_U32 height = 720;
    ...
    ret = kmpp_venc_chan_ctrl(id, MPP_ENC_SET_CFG, cfg);
...
void enc_test(void)
{
    ...
    RK_U32 width = 1280;
    RK_U32 height = 720;
    RK_U32 size = width * height * 3 / 2;
    RK_U32 hor_stride = width;
    RK_U32 ver_stride = height;

    kmpp_venc_init_cfg_get(&cfg);
    kmpp_venc_init_cfg_set_type(cfg, MPP_CTX_ENC);
    kmpp_venc_init_cfg_set_coding(cfg, MPP_VIDEO_CodingAVC);
    ...
    ret = kmpp_venc_chan_init(&id, cfg);
    if (ret) {
        kmpp_err_f("kmpp_venc_chan_init failed ret %d\n", ret);
        return;
    }

    ret = mpp_enc_cfg_setup(id);
    if (ret) {
    ...
    while (frame_count++ < frame_num) {
        ...
        kmpp_venc_chan_put_frm(id, frame);
    ...
    kmpp_venc_init_cfg_put(cfg);
    kmpp_venc_chan_stop(id, &cfg);
    kmpp_venc_chan_deinit(id, &cfg);
    mpp_enc_cfg_api_deinit();
}

rk_s32 kmpi_enc_chan_test_func(void *param)
{
    enc_test();
...
void kmpi_enc_chan_test_prepare(void)
{
    kmpp_logi("kmpp enc test start\n");
}

void kmpi_enc_chan_test_finish(void)
{
    kmpp_logi("kmpp enc test end\n");
}

int kmpi_enc_chan_test_init(void)
{
    kmpi_enc_chan_test_prepare();

    osal_worker_init(&test_worker, "kmpp enc test");
    osal_work_init(&test_work, kmpi_enc_chan_test_func, NULL);
...
void kmpi_enc_chan_test_exit(void)
{
    osal_worker_deinit(&test_worker);
    osal_work_deinit(&test_work);

    kmpi_enc_chan_test_finish();
}

module_init(kmpi_enc_chan_test_init);
module_exit(kmpi_enc_chan_test_exit);
